...
private:
    ...
    string NomeJogador;
    int pontuacao;
    char** matrizCampo; // Modificado para alocação dinâmica
    bool** camposRevelados; // Modificado para alocação dinâmica
...
public:
    ...
    void criarMatrizCampo() {
        matrizCampo = new char*[Comprimento];
        for (int i = 0; i < Comprimento; ++i) {
            matrizCampo[i] = new char[Largura];
    ...
    void criarCamposRevelados() {
        camposRevelados = new bool*[Comprimento];
        for (int i = 0; i < Comprimento; ++i) {
            camposRevelados[i] = new bool[Largura];
            for (int j = 0; j < Largura; ++j) {
                camposRevelados[i][j] = false;
            }
        }
    }
    ...
    int calcularPontuacao(int tempoDecorrido, double Densidade) const {
        ...
        return (Comprimento * Largura * pontuacaoBase) - (tempoDecorrido * pontuacaoTempo);
    }
